 # mqqt-temp-sens
A complete IoT solution for temperature and humidity sensing using MQTT protocol.

## Overview
`mqqt-temp-sens` is an IoT project that utilizes the popular [HTU21D](https://www.honeywell.com/us/en/search?q=htu21d) sensor to collect temperature and humidity data and sends it via MQTT protocol to a specified broker. The solution also includes a captive portal for easy configuration of network settings and MQTT credentials.
...
### Running the Project
After setting up your hardware and configurations, you can run the project by executing `main.py` on your MicroPython device:
